...
<template>
  <v-container class="w-[80vw] px-4 m-a text-center">
    <h3>訂單資訊</h3>
    <v-expansion-panels v-model="panel" multiple class="w-80% m-a mt-4">
      <v-expansion-panel v-for="i in data">
        <v-expansion-panel-title>
          <v-row class="items-center">
            <v-col><v-icon :icon="icon_change(i.status)"></v-icon></v-col>
            <v-col>訂單編號 : {{ i.id }}</v-col>

            <v-col>下單時間 : {{ i.created_at }}</v-col>
            <v-col> 狀態 : {{ i.status }}</v-col>
          </v-row>
        </v-expansion-panel-title>
        <v-expansion-panel-text class="border-t">
          <v-list class="p-0 h-a">
            <v-row class="w-[100%] h-[40px] my-2 items-center">
              <v-col> </v-col>
          ...
          <v-list v-for="it in i.items" class="p-0 my-2 h-a">
            <v-row class="w-[100%] h-[70px] my-2 items-center">
              <v-col>
